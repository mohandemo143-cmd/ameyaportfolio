    ...
    >
      <div className="glass flex items-center gap-2 rounded-full px-3 py-2">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {items.map((i) => (
            <a key={i} href={`#${i.toLowerCase()}`} className="rounded-full px-3 py-1.5 text-sm text-foreground/70 transition hover:bg-foreground/5 hover:text-foreground">{i}</a>
          ))}
        </nav>
        <Magnetic>
          <a href="#contact" className="ml-1 inline-flex items-center gap-2 rounded-full bg-foreground px-4 py-2 text-sm font-medium text-background transition hover:bg-foreground/90">
